...
namespace BTThietKeForm
{
    public partial class frmBai4 : Form
    {
        ...
        {
            Random rand = new Random(); int so;
            for (int i = 1; i <= 10; i++)
            {
                so = rand.Next(1,100); // Tạo số ngẫu nhiên > 1 và < 100
                listBox.Items.Add(so);
            }
        ...
        private void btnTimSo_Click(object sender, EventArgs e)
        {
            ...
            foreach (int so in listBox.Items)
            {
                if (so == soCanTim)
                {
                ...
                else lblKetQua.Text = "Không tìm thấy";
